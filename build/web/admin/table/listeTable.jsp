...
<body class="bg-gray-100 font-sans">
            ...
            <nav class="overflow-y-auto h-[calc(100vh-4rem)]">
                ...
                <div class="border-b border-gray-900/10">
                    ...
                    <div class="bg-gray-700">
                        <a href="./nouveauTable.html" class="block px-8 py-2 text-gray-200 hover:bg-gray-600">Nouveau</a>
            ...
            <main class="p-6 mt-16 flex-1 overflow-y-auto">
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        ...
                        %>
                        <tr>
                            <td class="px-6 py-4 whitespace-nowrap"><%=rs.getString("idtable")%></td>
                            <td class="px-6 py-4 whitespace-nowrap"><%=rs.getString("designation")%></td>
                            
                            <td class="px-6 py-4 whitespace-nowrap">
                                <% boolean occ = rs.getBoolean("occupation");
                                if(occ){
                                %>
                                    <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-red-100 text-red-800">Occupé</span>
                                <%
                                } else {
                                %>
                                    <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 text-green-800">Libre</span>
                                <%
                                }
                                %>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <button onclick="openEditModal(
                                    ...
                                    '<%= rs.getString("designation").replace("'", "\\'") %>',
                                    '<%= rs.getBoolean("occupation") %>'
                                )"
                                class="px-4 py-2 font-medium text-white bg-blue-600 rounded-md hover:bg-blue-500 transition">
                                    Edit
                                </button>
                                <button onclick="supprimeTable('<%= rs.getString("idtable")%>')" class="ml-2 px-4 py-2 font-medium text-white bg-red-600 rounded-md hover:bg-red-500 focus:outline-none focus:shadow-outline-red active:bg-red-600 transition duration-150 ease-in-out">Delete</button>
                            </td>
                        </tr>
                        <% } %>
                    </tbody>
    ...
    <div id="editModal" class="fixed inset-0 bg-black bg-opacity-50 hidden items-center justify-center z-50">
        <div class="bg-white p-6 rounded-lg shadow-md w-full max-w-md">
            <h2 class="text-xl font-semibold mb-4">Modifier la table</h2>
            <form id="editForm">
                <input type="hidden" id="edit_idtable" name="idtable" />
                <div class="mb-4">
                    <label for="edit_designation" class="block text-gray-700">Nom de la table</label>
                    <input type="text" id="edit_designation" name="designation" class="w-full px-3 py-2 border rounded" required>
                </div>
                <div class="mb-4">
                    <label for="edit_occupation" class="block text-gray-700">Occupation</label>
                    <select id="edit_occupation" name="occupation" class="w-full px-3 py-2 border rounded" required>
    ...
    <script>
    function supprimeTable(idtable) {
        console.log("Suppression de " + idtable);

        fetch("supprimeTable.jsp?idtable=" + encodeURIComponent(idtable))
            .then(res => res.text())
            .then(msg => {
                location.reload();
            });
        console.log("URL:", "supprimeTable.jsp?idtable=" + encodeURIComponent(idtable));
    }
    
    function openEditModal(id, designation, occupation) {
        document.getElementById("edit_idtable").value = id;
        ...
        e.preventDefault();
        const id = document.getElementById("edit_idtable").value;
        ...
        fetch("editTable.jsp", {
            ...
            body: "idtable=" + encodeURIComponent(id) +
